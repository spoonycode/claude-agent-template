...
- **Only create, read, edit, or link branches on Jira issues and Confluence pages within these spaces.**
  Never touch issues or pages belonging to a different project or space.
- Do NOT add comments to Jira tickets — record notes in git (commit messages, PR descriptions, branch comments) and add the branch link to the Jira issue instead.
...
- You may ONLY create, edit, transition, or add branch links on Jira issues assigned to `{{ASSIGNEE_EMAIL}}`
...
| `/designer` | HTML/CSS, UI, accessibility, design tokens (run separately, not per-story) |
...
2. **Architecture pre-check** — run `/architecture-pre-check`
3. **Branch** — create a branch named after the ticket key
4. **Design pre-check** — for UI/frontend stories only, run `/design-pre-check`
5. **Implement** — delegate to `/senior-developer`
6. **Test** — delegate to `/qa-engineer` after implementation is complete; move ticket to Testing column
7. **Security review** — delegate to `/security-tester` if auth, input, or data storage was touched
8. **Definition of Done** — run `/definition-of-done`; resolve any gaps before proceeding
9. **Pull request** — open a PR to `main`; never merge directly
10. **Jira** — transition the ticket to Done only after PR is created and DoD is fully met
