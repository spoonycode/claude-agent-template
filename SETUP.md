...
### 2. Fill in `.mcp.json` (6 values, same for every server)
...
| `YOUR_JIRA_API_TOKEN` | from https://id.atlassian.com/manage-profile/security/api-tokens |
| `YOUR_CONFLUENCE_API_TOKEN` | same token works for both |
| `YOUR_JIRA_PROJECT_KEY` | e.g. `MYAPP` |
| `YOUR_CONFLUENCE_SPACE_KEY` | e.g. `MyAppSpace` |

The same 6 values appear in all 6 server blocks — find/replace works well here.

### 3. Fill in `.claude/CLAUDE.local.md`
```
JIRA_PROJECT="MYAPP"
CONFLUENCE_SPACE="MyAppSpace"
ASSIGNEE_EMAIL="Agent-Claude"
```

### 4. Update `.claude/commands/project-brief.md`
Replace the placeholder text with a 2-3 sentence description of your project.

### 5. Load the MCP servers in Claude Code
In Claude Code, run `/mcp` and connect each of the 6 servers defined in `.mcp.json`.
...
| File | Purpose |
|---|---|
| `.mcp.json` | Configures 6 Jira MCP server instances, each scoped to this project via `JIRA_PROJECTS_FILTER` |
| `.claude/CLAUDE.md` | Orchestration rules — do not edit per project |
| `.claude/CLAUDE.local.md` | **Edit this per project** — project key, space key, assignee |
